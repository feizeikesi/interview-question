...
- 反编译`Task.Delay()`，基本上讲它就是个包裹在任务中的定时器。
- `Task.Delay()`和`Thread.Sleep()`最大的区别是`Task.Delay()`旨在异步运行，在同步代码中使用`Task.Delay()`是没有意义的；在异步代码中使用`Thread.Sleep()`是一个非常糟糕的主意。通常使用`await`关键字调用`Task.Delay()`。
